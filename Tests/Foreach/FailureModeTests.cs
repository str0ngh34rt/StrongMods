...
public class FailureModeTests {
  ...
  public void A_bad_xpath_is_an_error() {
    ...
    Exception onForeach = Assert.ThrowsAny<Exception>(() => PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item[" as="item">
        <append xpath="/items"><nope /></append>
      </foreach>
      """));
    ...
    PatchOutcome onBody = PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item" as="item">
  ...
  public void A_malformed_expression_is_an_error() {
    ...
    PatchOutcome unbalanced = PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item" as="item">
        <append xpath="/items">
      ...
      """);
    PatchOutcome unterminated = PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item" as="item">
        <append xpath="/items">
          <nope value="{$item/property[@name='Tier]/@value}" />
        </append>
  ...
  public void Function_arguments_may_not_use_a_fallback_or_nest_calls() {
    ...
    // other functions."
    PatchOutcome withFallback = PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item" as="item">
      ...
      """);
    PatchOutcome nested = PatcherHost.Instance.Value.Apply(TwoItems, """
      <foreach xpath="/items/item" as="item">
        <function name="up" method="Tests.FunctionMod.PatchFunctions.Upper, Tests.FunctionMod" />
        <append xpath="/items">

...
| `<dir>/AGENTS.md`            | Everyone, one directory subtree           | Instructions that apply only inside that subtree; prefer the repository root unless the rule is genuinely local                                                                          |
...
| `<store>/<repo>/AGENTS.md`   | The user, this repository, every machine  | Personal project instructions that follow the user across machines; `<store>` is the private synced store defined in setup.md (`~/agents/` here)                                         |

...
namespace StrongMods {
  ...
  public static class XmlPatchMethodForeach {
    ...
    // Active scopes, outermost first. World XML patching is single threaded at load; if that
    ...
    public static int Foreach(
      ...
      if (sourceAttribute != null) {
        ...
        if (!BreadthFirstXmlPatcher.TryGetPatchedFile(sourceName, out sourceFile)) {
          // The patched-file dictionary is keyed without the extension. A ".xml" suffix is the most
          // likely reason a lookup misses, so point straight at the fix rather than just reporting
          // the name back.
